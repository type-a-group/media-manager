...
page.on('request', (r) => {
	const u = r.url();
	if (u.includes('/api/classes/') || u.includes('/api/files?') || u.endsWith('/api/files'))
...
async function snap(label) {
	...
	const checks = await page
		.locator('[id^="cls-"]')
		.evaluateAll((els) => els.map((e) => ({ id: e.id, checked: e.getAttribute('aria-checked') })));
	console.log(`\n## ${label}`);
...
try {
	...
	await snap('initial All Files');
	await page.locator('#cls-documents').click();
	await snap('check Documents (solo)');
	await page.locator('#cls-travel').click();
	await snap('also check Travel (docs+travel)');
	await page.locator('#cls-documents').click();
	await snap('uncheck Documents (travel only?)');
} finally {
