 ...
 */
export type MediaTypeKind = 'images' | 'json';
...
export function readMediaTypeSettingsFileSync(baseDir: string): MediaTypeSettingsFile | null {
	...
	try {
		...
		const kind = parsed.kind === 'images' || parsed.kind === 'json' ? parsed.kind : null;
		const schema = parsed.schema && typeof parsed.schema === 'object' && !Array.isArray(parsed.schema) ? (parsed.schema as SchemaDefinition) : null;
		if (!kind || !schema) return null;
