								...
								<div class="flex">
									...
									<div>
										<h3 class="text-sm font-medium text-amber-800 dark:text-amber-200">
											Extension doesn't match image data
										</h3>
										<p class="text-sm text-amber-700 dark:text-amber-300 mt-1">
											This file has a <code class="px-1 py-0.5 rounded bg-amber-500/20 text-xs"
												>{metadata.fileExtension}</code
											>
											extension but the content appears to be {formatLabel(
												metadata.detectedFormatExtension
											)}.
										</p>
										<Button
											variant="outline"
								...
								<div>
									...
									<p class="text-sm text-muted-foreground mt-1">
										...
										{:else if metadata.format === 'jpeg' || metadata.format === 'jpg'}
											JPEG files can contain rich EXIF data including camera settings, GPS
											coordinates, and creation details.
										{:else}
							...
							<Collapsible.Content>
								{#if metadata.exif && Object.keys(metadata.exif).length > 0}
									<div class="rounded-md p-4 bg-muted">
										<pre class="text-sm text-foreground whitespace-pre-wrap">{formatValue(
												metadata.exif
											)}</pre>
									</div>
								{:else}
										...
										<div class="space-y-2">
											...
											<div class="mt-4">
												...
												<dd class="rounded-md p-3 bg-card">
													<pre
														class="text-xs text-foreground whitespace-pre-wrap break-all max-h-32 overflow-y-auto">{metadata
															.rawExif.buffer}</pre>
												</dd>
											</div>
										</div>
								...
								{:else if metadata.exif && Object.keys(metadata.exif).length > 0}
									<div class="rounded-md p-4 bg-muted">
										<p class="text-xs text-muted-foreground mb-2">
											Parsed tags (exiftool-vendored; no raw EXIF buffer). All metadata read from
											file:
										</p>
										<pre
											class="text-xs text-foreground whitespace-pre-wrap max-h-64 overflow-y-auto">{formatValue(
												metadata.exif
											)}</pre>
									</div>
								{:else}
									<div class="rounded-md p-4 border bg-muted">
										<div class="text-center">
											...
											<p class="text-xs text-muted-foreground mt-1">
												This image format doesn't contain raw EXIF data
											</p>
										</div>
									</div>
								{/if}
